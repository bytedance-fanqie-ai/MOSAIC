    ...
    **kwargs: dict,
    ...
    for i, query in enumerate(queries):
        ...
        if get_attn_maps and i >= 2:
            attn_score = torch.einsum("bhnd,bhmd->bhnm", query, key_img)  # [bs, num_heads, q_tokens, dims // heads] * [bs, num_heads, k_tokens, dims // heads]
            attn_map = F.softmax(attn_score / (head_dim ** 0.5), dim=-1)
            attn_map = attn_map.mean(dim=1)

            attn_maps.append(attn_map) # [1, 1024, 1024]
    ...
    if get_attn_maps:
        attn_maps = torch.cat(attn_maps, dim=1) # [1, 3072, 1024]
...
def block_forward(
    ...
    **kwargs: dict,
    ...
    if get_attn_maps:
        img_attn_output, txt_attn_output, attn_maps = attn_outputs
    else:
        img_attn_output, txt_attn_output = attn_outputs
    ...
    if get_attn_maps:
        return image_out, text_out, attn_maps
    else:
...
def single_block_forward(
    ...
    **kwargs: dict,
    ...
    if get_attn_maps:
        attn_outputs, attn_maps = attn_outputs
    else:
    ...
    **kwargs: dict,
    ...
    for block in self.transformer_blocks:
        ...
        if self.training and self.gradient_checkpointing:
            if get_attn_maps:
                image_hidden_states, text_hidden_states, attn_map = torch.utils.checkpoint.checkpoint(
            ...
            if get_attn_maps:
                image_hidden_states, text_hidden_states, attn_map = block_forward(**block_kwargs)
            else:
    ...
    for block in self.single_transformer_blocks:
        block_kwargs = {
            ...
            "adapters": adapters,
            "attn_forward": attn_forward,
            "get_attn_maps": get_attn_maps_single,
            **kwargs,
        }
        if self.training and self.gradient_checkpointing:
            if get_attn_maps_single:
                all_hidden_states, attn_map = torch.utils.checkpoint.checkpoint(
